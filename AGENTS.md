...
Integration tests for the **Digital Meter OCR API** (v2.7.0 — `wayu199/meter-ocr:v4`), which reads meter values from images using AI/OCR. Tests run against the API configured in `.env`.
...
> Tests that wait for async callbacks (`test_*_callback_received`, `test_get_task_completed_after_callback`) can take up to **2 minutes** (`CALLBACK_WAIT` default). Skip them without a tunnel: `pytest tests/ -v -k "not callback_received and not completed_after"`
...
The server is exposed publicly at `PUBLIC_URL` — this domain must be reachable from the internet for callback tests to work. `IMAGE_URL` and `CALLBACK_URL` are both constructed from this public URL.
...
| `PUBLIC_URL` | `PUBLIC_URL` | — (required for callback tests) |
| `CALLBACK_PORT` | `CALLBACK_PORT` | `3456` |
| `SYNC_TIMEOUT` | `SYNC_TIMEOUT` | `60` s |
| `ASYNC_TIMEOUT` | `ASYNC_TIMEOUT` | `30` s |
| `CALLBACK_WAIT` | `CALLBACK_WAIT` | `120` s |
| `IMAGE_PATH` | `IMAGE_PATH` | `image/example.jpg` |
